...
__global__ void build2(const char *text,int *pos, int *buffer, int text_size, int *offset){
	...
	if(idx<text_size){
		...
		//pos[idx]=head;
		pos[idx]=(head<=0)?idx+1:idx-head;
		if(pos[idx]>idx+1) pos[idx]=idx;
	}
}
__global__ void build(const char *text,int *pos, int *buffer, int text_size, int *offset){
	...
	if(idx<text_size){
		...
		while (((idx%2) ==0)){
			...
			buffer[offset_next+idx/2]=(buffer[offset_now+idx]>0 && buffer[offset_now+idx+1]>0)?1:0;
			__syncthreads();
			idx=idx/2;
			if(idx==0)
				break;
